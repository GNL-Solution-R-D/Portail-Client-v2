...
// Elle filtre la liste des membres, désormais alimentée par les ORGANIZATIONS
// de Keycloak (data/portail_api.php ?action=team.list →
// include/keycloak_organizations.php → Admin REST). Keycloak est la source de
...
<body class="bg-background text-foreground">
      ...
      <div class="app-shell-offset-min-height w-full bg-surface p-6 space-y-6">
        <div class="bg-background text-card-foreground flex flex-col gap-3 rounded-xl border py-6 shadow-sm">
          ...
          <div class="px-6 flex flex-wrap items-center gap-3 text-sm text-muted-foreground">
            ...
            <span id="orgBadge" class="inline-flex items-center justify-center rounded-md border px-2 py-0.5 text-xs font-medium bg-slate-100 text-slate-700 dark:bg-slate-800 dark:text-slate-300" hidden></span>
  ...
  <!-- Membres : ORGANIZATIONS Keycloak via data/portail_api.php ?action=team.list
       (include/keycloak_organizations.php → Admin REST). Lecture seule.
  ...
  (function () {
    ...
    ready(function () {
      ...
      var COLS  = 4;
      var state = { members: [], structure: '', organization: null, truncated: false };
      ...
      function updateHeader(){
        ...
        // Organisation Keycloak réellement interrogée (alias/nom technique).
        if (orgBadge) {
          var org = state.organization || {};
          var tech = org.alias || org.name || '';
          if (tech && tech !== state.structure) {
            orgBadge.textContent = tech;
            orgBadge.hidden = false;
          } else {
            orgBadge.textContent = '';
            orgBadge.hidden = true;
          }
        }
